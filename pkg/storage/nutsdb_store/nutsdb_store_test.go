// These tests seem to corrupt the DB and need clearing out
...
const (
	TestBoltDir        = "/tmp/nutsdb"
	TestBoltBucketName = "nutsdb_test"
)

func freshStore() *NutsDBStore {
	store, err := NewNutsDBStore(TestBoltDir, TestBoltBucketName)
	if err != nil {
		panic(fmt.Sprintf("Erroring opening NutsDB: %s", err))
	}
	store.Flush()
	return store
}
...
func BenchmarkInsertNoSync(b *testing.B) {
	...
	opt.Dir = fmt.Sprintf("%s_nosync", store.dir)
	// Todo: test this!
	opt.EntryIdxMode = nutsdb.HintBPTSparseIdxMode
	opt.SyncEnable = false

	store.DB, _ = nutsdb.Open(opt)
